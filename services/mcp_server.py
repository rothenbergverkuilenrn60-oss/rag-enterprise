...
async def _run_server() -> None:
    ...
    try:
        from mcp import (
            types,  # type: ignore[import-not-found]  # why: mcp package has no stubs as of 2026-05
        )
        from mcp.server import (
            Server,  # type: ignore[import-not-found]  # why: mcp package has no stubs as of 2026-05
        )
        from mcp.server.stdio import (
            stdio_server,  # type: ignore[import-not-found]  # why: mcp package has no stubs as of 2026-05
        )
    except ImportError:
        raise RuntimeError(
