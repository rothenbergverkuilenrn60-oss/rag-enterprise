...
### Default integration suite behavior

Integration tests run without bge-m3 / bge-m3-rerank on disk because
`tests/integration/conftest.py::_mock_local_model_inits` is an
`autouse=True` fixture (Phase 30-02 / TEST-INFRA-01) that
`patch.object(...)`'s both `HuggingFaceEmbedder.__init__` and
`CrossEncoderReranker.__init__` to no-op stubs.

The stub `__init__` assigns a `MagicMock` to `self._model` with `.encode`
/ `.predict` returning fixed-shape values (1024-d zero-ish vector,
scalar 0.5 score). This lets `AgentQueryPipeline` and `Retriever`
construct under a normal `pytest -m integration` invocation without
any model files present — the autouse mock fires for every integration
test by default.

Mock-at-consumer convention (v1.3 D-mock) is preserved: the patch
targets are `services.vectorizer.embedder.HuggingFaceEmbedder.__init__`
and `services.retriever.retriever.CrossEncoderReranker.__init__`
(consumer-path imports inside the test module aliases), not the
upstream `sentence_transformers` constructors.

### Real-embedder opt-out

**When to use:** a functional test that needs the *real* embedding /
reranking codepath (real shape, real similarity, real device
selection) rather than the mock. Example: a smoke test that verifies
the embed → vector-store → rerank pipeline against ground-truth model
behaviour, or a regression test guarding a model-version bump.

**How to mark:** stack both markers on the test (or module-level
`pytestmark`):
...
The autouse fixture inspects `request.node.get_closest_marker(
"real_embedder")`; when non-None it `yield`s without entering the
`patch.object(...)` block, exposing the real `__init__` methods.

**Environment requirements:** `$APP_MODEL_DIR` must contain the two
model trees so `config.settings.resolve_embedding_model_path("bge-m3")`
and `resolve_embedding_model_path("bge-m3-rerank")` resolve to existing
directories. See [Ops §4 — bge-m3 model dir layout](#4-bge-m3-model-dir-layout-td-07--phase-26)
for the three accepted layouts (HF vanilla, legacy `embedding_models/`,
HF hub cache).

**How to run** (canary, ships with v1.9 Hardening Round 3):

```bash
uv run pytest tests/integration/test_real_embedder_canary.py -m real_embedder
```

The canary instantiates both classes, runs `embed_batch(["hello"])`,
asserts a 1024-d vector, then `_model.predict([("q","d")])` and asserts
a scalar float score.

**Skip behavior:** if either model directory is absent on the host,
the canary file's module-level
`pytest.mark.skipif(not _models_present(), reason="bge-m3 /
bge-m3-rerank not present at $APP_MODEL_DIR; see docs/RUNBOOK.md")`
produces a `skipped` status — NOT an error. This keeps the integration
baseline error count clean on CI hosts that lack the files.

Default-suite runs (`pytest -m 'integration and not real_llm and not
real_embedder and not benchmark'`) deselect this marker, so the canary
neither runs nor affects the standard pass/fail/skip/error counts.
