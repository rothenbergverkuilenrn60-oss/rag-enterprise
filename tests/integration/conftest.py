...
def _mock_local_model_inits(request: pytest.FixtureRequest) -> object:
    ...
    # TEST-08 / D-OPTOUT-01: tests marked @pytest.mark.real_embedder opt out of
    # the autouse mock so real HuggingFaceEmbedder + CrossEncoderReranker
    # constructors run. Fixture is a generator-based autouse — must yield then
    # return (a bare `return` before any `yield` errors at collection time).
